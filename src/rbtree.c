...
void InorderTreePrint(rb_red_blk_tree* tree, rb_red_blk_node* x) {
  ...
  if (x != tree->nil) {
    ...
    tree->PrintKey(x->key);
    log_wr(DLOG, "  l->key=");
    if( x->left == nil) log_wr(DLOG, "NULL"); else tree->PrintKey(x->left->key);
    log_wr(DLOG, "  r->key=");
    if( x->right == nil) log_wr(DLOG, "NULL"); else tree->PrintKey(x->right->key);
    log_wr(DLOG, "  p->key=");
    if( x->parent == root) log_wr(DLOG, "NULL"); else tree->PrintKey(x->parent->key);
    log_wr(DLOG, "  red=%i\n",x->red);
